...
static int __mount_legacy_app_path(const char *app_root_path,
			const char *pkgid)
{
...
static int __mount_legacy_media_path()
{
...
static int __exec_app_process(void *arg)
{
	...
	if (home_dir) {
		if (!strncmp(launch_arg->menu_info->root_path,
			...
			ret = __mount_legacy_app_path(launch_arg->menu_info->root_path,
	...
	if (__prepare_exec(launch_arg->appid,
			launch_arg->app_path,
