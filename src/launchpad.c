...
static candidate_process_context_t* __add_slot(int type, int associated_pid, const char *loader_path);
static int __remove_slot(int type, int associated_pid);
...
static candidate_process_context_t* __find_slot_from_associated_pid(int pid)
{
	GList *iter = candidate_slot_list;

	while (iter) {
		candidate_process_context_t *cpc = (candidate_process_context_t*)iter->data;
		if (pid == cpc->associated_pid)
			return cpc;
...
static candidate_process_context_t* __find_slot(int type, int associated_pid)
{
	if (type == LAUNCHPAD_TYPE_DYNAMIC)
		return __find_slot_from_associated_pid(associated_pid);
...
static int __listen_candidate_process(int type, int associated_pid)
{
...
static int __get_launchpad_type(const char* internal_pool, const char* hwacc, const char *comp_type)
{
	if (strcmp(comp_type, "widgetapp") == 0)
		return LAUNCHPAD_TYPE_DYNAMIC;
...
static int __get_associated_pid(int type, bundle *kb)
{
	const char *val;
	...
	val = bundle_get_val(kb, AUL_K_ASSOCIATED_PID);
	if (val == NULL) {
		_E("failed to get associated pid");
		return -1;
	}
...
static int __prepare_candidate_process(int type, int associated_pid)
{
	int pid;
	char type_str[2] = {0, };
	char apid_str[10] = {0, };
	char *argv[] = {NULL, NULL, NULL, NULL};
	candidate_process_context_t* cpt = __find_slot(type, associated_pid);
	...
	if (pid == 0) { /* child */
		...
		type_str[0] = '0' + type;
		snprintf(apid_str, sizeof(apid_str), "%d", associated_pid);
...
static gboolean __handle_preparing_candidate_process(gpointer user_data)
{
	...
	__prepare_candidate_process(cpc->type, cpc->associated_pid);
...
static int __poll_fd(int fd, gushort events, GSourceFunc func, int type, int associated_pid)
{
	int r;
	GPollFD *gpollfd;
	GSource *src;
	...
	lc->gpollfd = gpollfd;
	lc->type = type;
	lc->associated_pid = associated_pid;
...
static gboolean __handle_loader_client_event(gpointer data)
{
	...
	int type = lc->type;
	int associated_pid = lc->associated_pid;
	gushort revents = lc->gpollfd->revents;

	candidate_process_context_t *cpc = __find_slot(type, associated_pid);
	...
	if (!cpc->prepared) {
		if (__accept_candidate_process(fd, &client_fd, &client_pid) >= 0) {
			...
			cpc->source = __poll_fd(client_fd, G_IO_IN | G_IO_HUP,
							(GSourceFunc)__handle_loader_client_event, type, associated_pid);
			if (cpc->source < 0)
				close(client_fd);
		}
	} else {
...
static gboolean __handle_sigchild(gpointer data)
{
	...
	do {
		...
		if (cpc != NULL) {
			__prepare_candidate_process(cpc->type, cpc->associated_pid);
			candidate_process_context_t *c = __find_slot_from_associated_pid(siginfo.ssi_pid);
			if (c)
				__remove_slot(c->type, c->associated_pid);
		}
	} while (s > 0);
...
static gboolean __handle_launch_event(gpointer data)
{
	...
	SECURE_LOGD("hwacc : %s\n", menu_info->hwacc);
	type = __get_launchpad_type(menu_info->internal_pool, menu_info->hwacc, menu_info->comp_type);
	if (type < 0) {
		_E("failed to get launchpad type");
		goto end;
	}

	associated_pid = __get_associated_pid(type, kb);

	add_slot_str = bundle_get_val(kb, AUL_K_ADD_SLOT);
	if (add_slot_str)
		add_slot_str = strdup(add_slot_str);
	...
	if (pid > 0) {
		...
		if (add_slot_str) {
			candidate_process_context_t *cpc = __add_slot(LAUNCHPAD_TYPE_DYNAMIC, pid, add_slot_str);
			if (cpc)
				cpc->timer = g_timeout_add(2000, __handle_preparing_candidate_process, cpc);
		}
	}
...
static candidate_process_context_t* __add_slot(int type, int associated_pid, const char *loader_path)
{
	...
	if (__find_slot(type, associated_pid) != NULL)
		return NULL;

	cpc->type = type;
	cpc->prepared = false;
	cpc->pid = CANDIDATE_NONE;
	cpc->associated_pid = associated_pid;
	cpc->send_fd = -1;
	...
	fd = __listen_candidate_process(cpc->type, cpc->associated_pid);
	if (fd == -1) {
	...
	if (__poll_fd(fd, G_IO_IN, (GSourceFunc)__handle_loader_event, cpc->type, cpc->associated_pid) < 0) {
		close(fd);
		return NULL;
	}
...
static int __remove_slot(int type, int associated_pid)
{
	...
	while (iter) {
		...
		if (type == cpc->type && associated_pid == cpc->associated_pid) {
			if (cpc->pid > 0)
...
{
	if (__add_slot(LAUNCHPAD_TYPE_COMMON, 0, LOADER_PATH_DEFAULT) == NULL)
		return -1;
	if (__prepare_candidate_process(LAUNCHPAD_TYPE_COMMON, 0) != 0)
		return -1;

	if (__add_slot(LAUNCHPAD_TYPE_SW, 0, LOADER_PATH_DEFAULT) == NULL)
		return -1;
	if (__prepare_candidate_process(LAUNCHPAD_TYPE_SW, 0) != 0)
		return -1;

	if (__add_slot(LAUNCHPAD_TYPE_HW, 0, LOADER_PATH_DEFAULT) == NULL)
		return -1;
