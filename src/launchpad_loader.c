		...
		void *user_data)
{
	...
	pool = bundle_get_val(kb, AUL_K_INTERNAL_POOL);

	if (pool && strcmp(pool, "true") == 0 && hwacc) {
		if (strcmp(hwacc, "NOT_USE") == 0) {
			_D("launchpad type: S/W");
			__fini_window();
			elm_config_accel_preference_set("none");
			__init_window();
		} else if (strcmp(hwacc, "USE") == 0) {
			_D("launchpad type: H/W");
		} else if (strcmp(hwacc, "SYS") == 0) {
			if (__sys_hwacc == SETTING_HW_ACCELERATION_ON) {
				_D("launchpad type: H/W");
			} else if (__sys_hwacc == SETTING_HW_ACCELERATION_OFF) {
				_D("launchpad type: S/W");
				__fini_window();
				elm_config_accel_preference_set("none");
				__init_window();
